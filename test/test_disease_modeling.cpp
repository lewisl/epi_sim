...
void test_make_sick_updates_state_and_series() {
  ...
  assert(person.status() == INFECTIOUS);
  assert(person.cond() == NIL);
  assert(person.duration() == 1);
  assert(person.variant() == Variant{1});
  assert(person.sickday() == 1);
  assert(person.variant_hist().count == 1);
  assert(person.variant_hist().latest() == Variant{1});
  assert(person.sickday_hist().count == 1);
  assert(person.sickday_hist().latest() == 1);

  assert(series.new_status.at(uint8_t(INFECTIOUS), AgeBucket::total)[1] == 1);
  assert(series.now_status.at(uint8_t(INFECTIOUS), AgeBucket::total)[1] == 1);
  assert(series.now_status.at(uint8_t(UNEXPOSED), AgeBucket::total)[1] == 0);
  assert(series.new_variant.at(uint8_t(Variant{1}), AgeBucket::total)[1] == 1);
  assert(series.now_variant.at(uint8_t(Variant{1}), AgeBucket::total)[1] == 1);
}
...
void test_make_dead_sets_death_state() {
  ...
  assert(person.status() == DEAD);
  assert(person.deadday() == 1);
  assert(person.variant() == Variant{1});

  assert(series.new_status.at(uint8_t(DEAD), AgeBucket::total)[1] == 1);
  assert(series.now_status.at(uint8_t(DEAD), AgeBucket::total)[1] == 1);
  assert(series.now_status.at(uint8_t(INFECTIOUS), AgeBucket::total)[1] == 0);
  assert(series.now_variant.at(uint8_t(Variant{1}), AgeBucket::total)[1] == 0);
}
