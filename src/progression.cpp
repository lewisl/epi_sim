...
void progression(PopData &pop, size_t p, ProgressionSet &progset,
  ...
  auto p_agegrp = pop.agegrp[p];
  auto & p_cond = pop.cond[p];
  auto & p_status = pop.status[p];
  auto p_vaxstatus = pop.vaxstatus[p];  // won't update this
  ...
  if (probtree.at(p_agegrp - 1).contains(p_duration)) {
    ...
    std::copy(src.begin(), src.end(), probvec.begin());
...
*/
void redistribute_probability(vector<float> &probvec, float riskfactor, uint8_t duration) {
...
void do_progression(PopData &pop, size_t p, const vector<float> &probvec) {
  uint8_t outcome = xo::categorical_fast(probvec);

  // just for debugging output
  uint8_t prev_cond = pop.cond[p];

  if (outcome == Trait::Progressmap::dead) {
    pop.deadday[p] = sim::get_day();
    pop.status[p] = Trait::Stat::dead;   // todo will we need to set cond to uninfected for any other logic?
  } else if (outcome == Trait::Progressmap::recovered) {
    pop.make_well(p);
  } else {
  ...
  // just for debugging
  if (outcome == 5)
    fmt::println("======= Progression: Person {} Probvec: {} Raw outcome: {} from: {} to: {}", p, outcome, probvec, Trait::Progressionmap.to_str(prev_cond), Trait::Progressionmap.to_str(pop.cond[p]));
